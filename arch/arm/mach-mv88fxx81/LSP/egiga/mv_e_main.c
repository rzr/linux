...
#ifdef CONFIG_BUFFALO_PLATFORM
                printk( "'eth0' " );
                if( egiga_load( 0, "eth0", CONFIG_ETH_0_MACADDR, CONFIG_ETH_0_MTU, ETH_PORT0_IRQ_NUM ) )
                    err = 1;
#else
                printk( "'egiga0' " );
...
#endif
	if( port_status & BIT4 ){
		printk( KERN_NOTICE ", speed 1 Gbps" );
#ifdef CONFIG_BUFFALO_PLATFORM
		buffalo_struc.linkspeed=1000;
#endif
	}else{
		printk( KERN_NOTICE ", %s",(port_status & BIT5) ? "speed 100 Mbps" : "speed 10 Mbps" );
#ifdef CONFIG_BUFFALO_PLATFORM
...
#ifdef CONFIG_BUFFALO_PLATFORM
void egiga_buffalo_change_configuration(MV_U8 mode)
{
...
#if 1
	len += sprintf(buf+len, "regster9 value=0x%04x\n", reg);
#endif
	len += sprintf(buf + len, "mode.config=%s:%s\n",
		(reg & BIT12)? "manual":"auto",
		(reg & BIT11)? "master":"slave");
	mvEthPhyRegRead(mvBoardPhyAddrGet( 0 ), 10, &reg);
	len += sprintf(buf + len, "mode.actual=%s\n",
		(reg & BIT14)? "master":"slave");

#if 1
	len += sprintf(buf + len, "=== for debug ===\n");
	unsigned int adr, page;
	for(page=0; page<10; page++){
		mvEthPhyRegWrite(mvBoardPhyAddrGet(0), 22, page);
		len += sprintf(buf + len, "\n\npage=%d", page);
		for(adr=0; adr<32; adr++){
			if((adr!=0) && (adr % 8 == 0)){
				len += sprintf(buf + len, "\n      ");
			}
			mvEthPhyRegRead(mvBoardPhyAddrGet(0), adr, &reg);
			len += sprintf(buf + len, " 0x%04x", reg);
		}
	}
#endif
	return len;
