...
//#define TRACE_IRQ  do {if(irq>=32) printk(">%s: IRQ(%d)\n",__FUNCTION__,irq);} while(0)
#define TRACE_IRQ
...
static void mv_mask_irq(unsigned int irq)
{
	...
	/*
		...
		printk(" gpp cause is %x , mask %x lvl=%x\n",MV_REG_READ(MV_GPP_IRQ_CAUSE_REG), MV_REG_READ(MV_GPP_IRQ_MASK_REG), MV_REG_READ(MV_GPP_IRQ_MASK_REG+4) );
