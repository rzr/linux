...
different major-minor numbers for that disk, exactly as if it was new disk, so in the customerfs test,
when the disk was re-connected it was identified as sdb (where initially it was sda).

Now, if the Buffalo doesnft like this behavior, then there is a simple workaround. The workaround is
simply to remove hot-plug support from the driver, so when a disk is suddenly disconnected then
reconnected the kernel will not be notified. Doing that, the kernel will not change the gdisk addressh.
...
1. If the test ran previously on older IDE system? (Ifm more interested in Linux kernel version
    + driver being used)
 
2. If the system that was tested doesnft have real hot swap capability and thus it is similar to
