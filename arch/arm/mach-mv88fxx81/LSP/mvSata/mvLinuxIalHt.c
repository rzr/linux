...
int mv_ial_ht_queuecommand (struct scsi_cmnd * SCpnt, void (*done) (struct scsi_cmnd *))
{
    ...
    else if (SCpnt->use_sg == 1) {
	struct scatterlist *sgpnt = (struct scatterlist *)SCpnt->request_buffer;
	completion_info->pSALBlock->pDataBuffer = (void *)((unsigned)phys_to_virt(page_to_phys(sgpnt->page)) + sgpnt->offset);
    }
    else {
	/* XXX: pDataBuffer is not used in this case (maybe) */
	completion_info->pSALBlock->pDataBuffer = SCpnt->request_buffer;
    }
