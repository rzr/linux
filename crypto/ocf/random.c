...
static inline _syscall3(int,ioctl,int,fd,unsigned long,cmd,const void *,arg);
static inline _syscall3(int,poll,struct pollfd *,pollfds,int,nfds,int,timeout);
...
random_proc(void *arg)
{
	...
	while (rfd != -1) {
#ifdef FIPS_TEST_RNG
#define NUM_INT32 (RNDTEST_NBYTES/sizeof(unsigned long))
#else
#define NUM_INT32 32
#endif
		static uint32_t		buf[2 + NUM_INT32];
		static int			bufcnt  = 0;
		struct random_op	*rops, *tmp;
		...
		if (bufcnt) {
			...
			buf[0] = NUM_INT32 * 32;
			buf[1] = NUM_INT32 * 4;
			(void) ioctl(rfd, RNDADDENTROPY, buf);
			bufcnt = 0;
		}

		while (bufcnt < NUM_INT32) {
			list_for_each_entry_safe(rops, tmp, &random_ops, random_list) {
